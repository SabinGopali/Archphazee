...
export default function Updateproduct() {
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      ...
      try {
        ...
        const res = await fetch(`/backend/product/get/${id}`);
        ...
        if (data.variants && data.variants.length > 0) {
          ...
          // Fill remaining slots with empty variants
          while (updatedVariants.length < 3) {
            updatedVariants.push({ name: "", images: [], existingImages: [] });
          }
  ...
  const handleVariantImageUpload = (index, files) => {
    ...
    const totalImages = (updated[index].images?.length || 0) + (updated[index].existingImages?.length || 0);
    const availableSlots = 3 - totalImages;
    updated[index].images = [...(updated[index].images || []), ...newImages.slice(0, availableSlots)];
    setVariants(updated);
  };

  // Remove variant image (new)
  ...
  const removeVariantExistingImage = (vIdx, iIdx) => {
    const updated = [...variants];
                ...
                {variants.map((variant, index) => (
                  ...
                  >
                    ...
                    <div className="grid grid-cols-3 gap-4">
                      ...
                      {/* Add image slot */}
                      {((variant.images?.length || 0) + (variant.existingImages?.length || 0)) < 3 && (
