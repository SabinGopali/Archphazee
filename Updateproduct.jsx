...
export default function Updateproduct() {
  ...
  // Helper function to construct image URL - updated to match ManageProduct pattern
  ...
  useEffect(() => {
    const fetchProductData = async () => {
      ...
      try {
        ...
        // Set existing images - updated to match ManageProduct pattern
        ...
        // Set existing variants - updated image processing
        if (data.variants && data.variants.length > 0) {
          const updatedVariants = [...variants];
          data.variants.forEach((variant, index) => {
            if (index < 3) {
              // Process variant images using the same pattern as ManageProduct
              const processedVariantImages = variant.images ? variant.images.map(img => {
                const imagePath = typeof img === 'string' ? img : (img.images || img);
  ...
  const handleAddVariant = () => {
    if (variants.length < 3)
      setVariants([...variants, { name: "", images: [], existingImages: [] }]);
  };
  ...
  const handleVariantImageUpload = (index, files) => {
    ...
    const existingCount = updated[index].existingImages?.length || 0;
    const currentCount = updated[index].images?.length || 0;
    const totalCount = existingCount + currentCount;
    const availableSlots = 3 - totalCount;
    
    updated[index].images = [...(updated[index].images || []), ...newImages.slice(0, availableSlots)];
    setVariants(updated);
  };
  ...
  const removeExistingVariantImage = (vIdx, iIdx) => {
    const updated = [...variants];
    updated[vIdx].existingImages.splice(iIdx, 1);
    setVariants(updated);
  };
                        ...
                        {existingImages.map((img, idx) => (
                          ...
                          >
                            <img
                              ...
                              onError={(e) => {
                                console.error("Image failed to load:", getImageUrl(img.images));
                ...
                {variants.map((variant, index) => (
                  ...
                  >
                    ...
                    <div className="grid grid-cols-3 gap-4">
                      ...
                      {variant.existingImages?.map((imgUrl, imgIdx) => (
                        <div
                          key={`existing-${imgIdx}`}
                        ...
                        >
                          <img
                            ...
                            onError={(e) => {
                              console.error("Variant image failed to load:", imgUrl);
                              e.target.src = "/placeholder-image.jpg"; // Fallback image
                            }}
                          />
                          <button
